...
class VideoCutterApp:
    ...
    def create_widgets(self):
        ...
        tk.Radiobutton(frame_opts, text="每一幀都輸出 (Frame by Frame)", 
                       variable=self.mode, value="all", command=self.toggle_interval_input).grid(row=0, column=0, sticky="w", padx=10, pady=5)
    ...
    def start_thread(self):
        ...
        if not os.path.exists(self.video_path.get()):
            messagebox.showerror("路徑錯誤", "找不到影片檔案，請檢查路徑！") # <--- 這裡會跳出錯誤視窗
            return
        if not self.output_folder.get():
            messagebox.showerror("路徑錯誤", "請選擇輸出資料夾！") # <--- 這裡會跳出錯誤視窗
            return

        # 3. 檢查數字輸入 (Try-Catch 防呆)
        if self.mode.get() == "interval":
            try:
                # 嘗試讀取數值。如果使用者輸入文字，這裡會直接報錯，跳到 except
            ...
            except tk.TclError: # 捕捉 TclError (這是 tkinter 遇到型別錯誤時會丟出的錯誤)
                messagebox.showerror("輸入錯誤", "您輸入的不是數字！\n請輸入正確的秒數 (例如: 0.5, 1, 5)") # <--- 錯誤視窗
                return
            except Exception as e: # 捕捉其他未知的錯誤
    ...
    def process_video(self):
        ...
        frame_step = 1
        if mode == "interval":
            # 這裡不需要再 try-catch 了，因為 start_thread 已經檢查過了
    ...
    def finish_processing(self, message, error=False):
        ...
        if error:
            ...
            messagebox.showinfo("成功", message) # <--- 成功也會跳出視窗
